...
`bdknatividad/SCH-PATH`, branch `master` (only branch). Live: `sch-path-production.up.railway.app`
(API) + `sch-path.vercel.app` (SPA). No MySQL locally — verify on the live pair. History lives in the
daily logs; this file is rules only.
...
- **Frontend pins.** `backend/tests/*.test.js` reads `.tsx` as text; renaming a state variable in a
  ...
  depends on indentation; `[^)]*` breaks on nested parens — use `[^;]*`. Pin the read path too.
  Re-run the suite after editing a pinned component.
- **Mutation discipline.** Neuter, confirm the test *fails*, restore from bytes captured at apply time
  (never `git checkout --`), unconditionally. Print `matched N occurrence(s)`, exit non-zero on zero.
  Mutate every file the contract spans. A node process cannot spawn the managed node binary — run the
  suite from bash.
- **`errorHandler` masks every MySQL error** (all `ER_*` → "Database error occurred"). A bare
  ...
  `toISOString()`. `ER_DATA_TOO_LONG` is masked: the bound is the declared width of the source column.
...
- **A NOT NULL column is a silent feature blocker.** `CREATE TABLE IF NOT EXISTS` does nothing to a
  ...
  try/catch + `console.warn`). Normalize nulls at every boundary that puts a row into state, including
  save responses. A nullable text column needs `blankToNull`.
- **`ensureColumn` vs `Object.entries`.** `schema-contract.test.js`'s `parseEnsureColumns` sees
  `ensureColumn('t','c',…)` and the `for (…of [...])` loop, but NOT an object literal iterated with
  `Object.entries` (`childColumnDefinitions`). A column the app writes that no parsed migration creates
  is missing on every deployed DB.
...
- **PDF geometry: three y conventions.** `pdfPercentTop` centres a box on a bottom-left centre;
  `pdfTop` takes a lower edge; `HOUSEPARENT_SIGNATURE_BOX.top` is a top-left upper edge. A rule's ink
  sits below its baseline. Crop the render — text extraction cannot tell whether a stamp landed on
  blank template.
...
- `VITE_API_URL` is baked at build time and **must** be set on Vercel. Railway redeploys on every push
  to `master`; Vercel can silently skip a push — check the commit's statuses.
