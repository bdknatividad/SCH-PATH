...
- **Frontend pins.** `backend/tests/*.test.js` reads `.tsx` as text, so renaming a state variable in a
  ...
  indentation; `[^)]*` breaks on nested parens — use `[^;]*`. Re-run the suite after editing a pinned
  component.
...
- **A resident-scoped filter must know how the resource is keyed.** `rowResidentIds()` reads
  ...
  and dropped **every** resident (`children: []`) while `GET /children` returned the real caseload.
  Fixed in `ab3fc51`, pinned by `tests/store-houseparent-caseload.test.js`. Hidden because `DataContext`
  overwrites the Houseparent list from `/resident-assignments/my-residents`.
- **Keying a link on a display name (recurring class).** Add an id column beside the name, read the id
  ...
  and `schema.sql`.
- **There is no API path back from `Absconded`** that does not fabricate an admission cycle:
  `PUT /children/:id` is refused by `assertResidentNotAbsconded`, and only `readmit` and the admissions
  create path write `children.status = 'Active'` — both insert an `admissions` row.
...
- **PDF geometry has three y conventions** (`pdfPercentTop` centres on a bottom-left centre, `pdfTop`
  takes a lower edge, `HOUSEPARENT_SIGNATURE_BOX.top` a top-left upper edge) and a rule's ink sits below
  its baseline — crop the render; text extraction cannot tell whether a stamp landed on blank template.
  And **a label is not a value**: a renamed option keeps its stored value.
...
- Lazy routes via `@/app/utils/lazyComponent`. **Never add `pdfjs-dist` to `manualChunks`**. Entry
  chunk budget ≈ 142 kB gzip (currently 111 kB / 32.9 kB gzip).
...
- `VITE_API_URL` is baked at build time, so **a local chunk hash never matches Vercel's** — compare two
  *deployments*, or grep the served bundle for string literals, never compare against a local build.
...
`EventSource`). Roles: `centerhead` (fullAccess), `admin`, `nurse`, `psychologist`, `educator`,
...
- **An empty `accessibleModules` means "fall back to the role's full matrix".** Read the role
  ...
  **silent revocation**.
- **A notification is invisible to its own actor** — `visibilityClause` ends with
  `actorUsername <> ?`. Never diagnose a "missing" notification from the actor's own account.
...
- **Caseload scope:** only `houseparent` is scoped (`utils/residentScope.js`), forward-only. Seed creds:
  ...
  `nurse`/`nurse123`, `educator`/`educator123`, `HP n`/`hp n 123`. **There is no `admin` account**;
  the live `socialworker` password is **not** `social123` — do not reset a real account to get a token.
- `/store` answers `{success, data}`. `/api/children` is ungated at the mount and scopes Houseparents
  inside the controller; `/store` has no `children` key in `STORE_MODULE_BY_RESOURCE` and relies on the
  scope filter. `/api/tri` (list) is gated by the **Houseparent** module while `/api/tri/monitor` and
  `/summary` are role-gated. A 404 on `GET /api/admissions`, `/resident-assignments`,
  `/discharge-plans` or `/incident-reports` is correct — those routers have no `GET /`.
...
- Behavioural controller tests without a DB: stub `src/config/database` in `require.cache`, then
  dispatch on the SQL text. The schema can be audited live without DB access because `mapRow` only emits
  `col in row`.
...
- Playwright lives in the managed node workspace (`channel: 'msedge'`); login rate-limits (429) — cache
  ...
  `/tmp` is `C:\tmp` but `$(cygpath -w /tmp)` is `%TEMP%`; restore a mutated file with
  `Path.write_bytes`. Heredocs piped to `node -e` get shell-mangled — write the script with the file tool.
