...
export function Activities() {
  ...
  const { children: allChildren, activities, addActivity, updateActivity, deleteActivity } = useData();
  const { user } = useAuth();
  const isPsychologist = user?.role === 'psychologist' || user?.role === 'centerhead';
  const liveResidents = allChildren.map(c => ({ id: c.id, name: c.name, caseType: c.caseType, age: c.age }));
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      updateActivity(editingId, activityData as any);
    } else {
      addActivity(activityData as any);
    }
    resetForm();
  };
