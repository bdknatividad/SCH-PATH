...
export default function ActivityDetail() {
  ...
  const { activities, children: allChildren, isLoading } = useData();
  const [activity, setActivity] = useState<any>(null);
  const [participants, setParticipants] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Wait for the store to finish loading before concluding anything. The
    // previous version bailed out early whenever `activities` was empty and
    // never cleared `loading`, so the spinner stayed on screen forever.
    if (isLoading) return;
    ...
    if (found) {
      ...
      setParticipants(
        ids.length > 0
          ? allChildren.filter(c => ids.includes(c.id)).map(c => ({ id: c.id, name: c.name }))
          : []
      );
    } else {
      // Store is loaded and this id is not in it — show the not-found state.
      setActivity(null);
      setParticipants([]);
    }
    setLoading(false);
  }, [id, activities, allChildren, isLoading]);
