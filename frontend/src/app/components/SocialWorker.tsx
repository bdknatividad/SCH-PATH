...
export function SocialWorker() {
  ...
  const handleAddNote = () => {
    // In a real implementation, this would save to a caseNotes table
    alert(`Case note saved for ${newCaseNote.resident}`);
    setNewCaseNote({ resident: '', caseType: 'Case Note', note: '' });
  };

  // Get resident violations count
  const getResidentViolationPoints = (residentId: string) => {
    return violations
        ...
        <TabsContent value="residents" className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {assignedResidents.map((resident) => {
              const violationPoints = getResidentViolationPoints(resident.id);
                    ...
                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          {violationPoints >= 5 && (
                            <Badge className={violationPoints >= 10 ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}>
                              {violationPoints} pts
                            </Badge>
                          )}
                        </div>
          ...
          {residentsWithViolations.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {residentsWithViolations.map((resident) => {
                ...
                const totalPoints = residentViolations.reduce((sum, v) => sum + v.points, 0);
                return (
                  <Card key={resident.id}>
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Badge className={totalPoints >= 10 ? 'bg-red-100 text-red-700' : totalPoints >= 5 ? 'bg-orange-100 text-orange-700' : 'bg-yellow-100 text-yellow-700'}>
                          {totalPoints} Points
                        </Badge>
                      </div>
                      <div className="space-y-2">
